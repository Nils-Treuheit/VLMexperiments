...
DOCVQA_MODELS = {"florence2", "paligemma", "llama_vision", "phi_vision", "cosmos_nemotron",
...
DOCVQA_QUESTIONS = [
    ("Is there any text visible in this image?", ["yes", "no"]),
    ("Is this image of a document, book, or sign with text?", ["yes", "no"]),
    ("Does the image contain numbers or digits?", ["yes", "no"]),
    ("Is this a screenshot or a photograph of a screen?", ["yes", "no"]),
    ("Does the image contain a menu, label, or instructions?", ["yes", "no"]),
    ("Is there a logo or brand name visible?", ["yes", "no"]),
    ("Does this image show a page from a book or magazine?", ["yes", "no"]),
    ("Is there a street sign or directional sign visible?", ["yes", "no"]),
    ("Is there a food label or product packaging with text?", ["yes", "no"]),
    ("Does the image contain a notice, warning, or announcement?", ["yes", "no"]),
]


def extract_docvqa_answer(text, choices):
...
def benchmark_docvqa(model_name, max_questions=200, verbose=True):
    mn = MODEL_ALIASES.get(model_name, model_name)
    if mn not in MODEL_LOADERS:
        raise ValueError(f"Unknown model {mn!r}")
    if mn not in DOCVQA_MODELS:
        raise ValueError(f"Model {mn!r} does not support DocVQA. Choose from: {DOCVQA_MODELS}")
    ...
    for img_id in img_ids:
        ...
        ow, oh = image.size
        sampled_qs = rng.sample(DOCVQA_QUESTIONS, min(q_per_img, len(DOCVQA_QUESTIONS)))
        ...
        for question, choices in sampled_qs:
            ...
            try:
                if is_f2:
                    ...
                    messages = [{"role": "user", "content": [
                        {"type": "image"},
                        {"type": "text", "text": f"Look at this image carefully and answer: {question}"}
                    ]}]
            ...
            predicted = extract_docvqa_answer(answer, choices)
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Document VQA Results: {display}")
        print(f"{'=' * 60}")
    ...
    save_stats(stats, f"{safe_name}_docvqa")
    return stats


def main():
    parser = argparse.ArgumentParser(description="Document VQA Benchmark (COCO)")
    all_choices = [m for m in list(MODEL_LOADERS) + list(MODEL_ALIASES)
                   if MODEL_ALIASES.get(m, m) in DOCVQA_MODELS or m in DOCVQA_MODELS]
    ...
    stats = benchmark_docvqa(args.model, max_questions=args.max_questions, verbose=True)
    if stats:
        print_comparison({args.model: stats}, "DOCUMENT VQA — COCO")
