...
REASONING_MODELS = {"florence2", "paligemma", "llama_vision", "phi_vision", "cosmos_nemotron",
...
REASONING_QUESTIONS = [
    ("Which is larger, the person or the vehicle in this image?", ["person", "vehicle", "neither"]),
    ("Is there a person using an electronic device?", ["yes", "no"]),
    ("Is there an animal near a person in this image?", ["yes", "no"]),
    ("Which object is most likely to be found on a table?", ["cup", "book", "bottle", "none"]),
    ("Is there a food item visible in this image?", ["yes", "no"]),
    ("Is a person interacting with an object?", ["yes", "no"]),
    ("Can you see both a person and a vehicle in this image?", ["yes", "no"]),
    ("Is there an object that could hold liquid?", ["yes", "no"]),
    ("Are there more than two people in this image?", ["yes", "no"]),
    ("Is there a sitting surface in this image?", ["yes", "no"]),
]


def extract_reasoning_answer(text, choices):
...
def benchmark_visual_reasoning(model_name, max_questions=200, verbose=True):
    mn = MODEL_ALIASES.get(model_name, model_name)
    if mn not in MODEL_LOADERS:
        raise ValueError(f"Unknown model {mn!r}")
    if mn not in REASONING_MODELS:
        raise ValueError(f"Model {mn!r} does not support visual reasoning. Choose from: {REASONING_MODELS}")
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Visual Reasoning: {display}")
        print(f"{'=' * 60}")
    ...
    for img_id in img_ids:
        ...
        ow, oh = image.size
        sampled_qs = rng.sample(REASONING_QUESTIONS, min(q_per_img, len(REASONING_QUESTIONS)))
        ...
        for question, choices in sampled_qs:
            ...
            try:
                if is_f2:
                    ...
                    messages = [{"role": "user", "content": [
                        {"type": "image"},
                        {"type": "text", "text": f"Answer the question with one word: {question}"}
                    ]}]
            ...
            predicted = extract_reasoning_answer(answer, choices)
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Visual Reasoning Results: {display}")
        print(f"{'=' * 60}")
    ...
    save_stats(stats, f"{safe_name}_visual_reasoning")
    return stats


def main():
    parser = argparse.ArgumentParser(description="Visual Reasoning Benchmark (COCO)")
    all_choices = [m for m in list(MODEL_LOADERS) + list(MODEL_ALIASES)
                   if MODEL_ALIASES.get(m, m) in REASONING_MODELS or m in REASONING_MODELS]
    ...
    stats = benchmark_visual_reasoning(args.model, max_questions=args.max_questions, verbose=True)
    if stats:
        print_comparison({args.model: stats}, "VISUAL REASONING — COCO")
