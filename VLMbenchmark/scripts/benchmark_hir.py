...
HIR_MODELS = {"florence2", "paligemma", "llama_vision", "phi_vision", "cosmos_nemotron",
...
HIR_QUESTIONS = [
    ("Is the person in this image interacting with an object?", ["yes", "no"]),
    ("Is the person reaching for or holding something?", ["yes", "no"]),
    ("Is the person likely preparing to move or walk?", ["yes", "no"]),
    ("Is the person looking at or reading something?", ["yes", "no"]),
    ("Is the person using a tool or device?", ["yes", "no"]),
    ("Is the person likely eating or drinking?", ["yes", "no"]),
    ("Is the person communicating with someone?", ["yes", "no"]),
    ("Is the person engaged in a sport or exercise activity?", ["yes", "no"]),
]

GT_HIR_TEMPLATES = {
    "person": [
        ("Is the person in this image interacting with an object?", "yes"),
        ("Is the person communicating with someone?", "no"),
    ],
}


def extract_hir_answer(text, choices):
...
def benchmark_hir(model_name, max_questions=200, verbose=True):
    mn = MODEL_ALIASES.get(model_name, model_name)
    if mn not in MODEL_LOADERS:
        raise ValueError(f"Unknown model {mn!r}")
    if mn not in HIR_MODELS:
        raise ValueError(f"Model {mn!r} does not support HIR. Choose from: {HIR_MODELS}")
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Human Intention Recognition: {display}")
        print(f"{'=' * 60}")
    ...
    for img_id in img_ids:
        ...
        ow, oh = image.size
        sampled_qs = rng.sample(HIR_QUESTIONS, min(q_per_img, len(HIR_QUESTIONS)))
        ...
        for question, choices in sampled_qs:
            ...
            try:
                if is_f2:
                    ...
                    messages = [{"role": "user", "content": [
                        {"type": "image"},
                        {"type": "text", "text": f"Look at the person and answer: {question}"}
                    ]}]
            ...
            predicted = extract_hir_answer(answer, choices)
    ...
    stats = {
        "model": display,
        "model_key": mn,
        "task": "human_intention_recognition",
        "dataset": "coco",
        "questions": total,
        "skipped": skipped,
    ...
    save_stats(stats, f"{safe_name}_hir")
    return stats


def main():
    parser = argparse.ArgumentParser(description="Human Intention Recognition Benchmark (COCO)")
    all_choices = [m for m in list(MODEL_LOADERS) + list(MODEL_ALIASES)
                   if MODEL_ALIASES.get(m, m) in HIR_MODELS or m in HIR_MODELS]
    ...
    stats = benchmark_hir(args.model, max_questions=args.max_questions, verbose=True)
    if stats:
        print_comparison({args.model: stats}, "HUMAN INTENTION RECOGNITION — COCO")
