...
EMOTION_MODELS = {"florence2", "paligemma", "llama_vision", "phi_vision", "cosmos_nemotron",
...
EMOTION_QUESTIONS = [
    ("Does the person in this image look happy?", ["yes", "no"]),
    ("Does the person in this image look sad?", ["yes", "no"]),
    ("Does the person in this image look surprised?", ["yes", "no"]),
    ("Does the person in this image look angry?", ["yes", "no"]),
    ("Does the person in this image look neutral or calm?", ["yes", "no"]),
    ("Is the person smiling?", ["yes", "no"]),
    ("Is the person showing a positive emotional expression?", ["yes", "no"]),
    ("Is the person showing a negative emotional expression?", ["yes", "no"]),
]

GT_EMOTION_TEMPLATES = {
    "person": [
        ("Is the person showing a positive emotional expression?", "yes"),
        ("Is the person showing a negative emotional expression?", "no"),
    ],
}
...
def benchmark_emotion(model_name, max_questions=200, verbose=True):
    mn = MODEL_ALIASES.get(model_name, model_name)
    if mn not in MODEL_LOADERS:
        raise ValueError(f"Unknown model {mn!r}")
    if mn not in EMOTION_MODELS:
        raise ValueError(f"Model {mn!r} does not support emotion detection. Choose from: {EMOTION_MODELS}")
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Emotion Detection: {display}")
        print(f"{'=' * 60}")
    ...
    for img_id in img_ids:
        ...
        ow, oh = image.size
        sampled_qs = rng.sample(EMOTION_QUESTIONS, min(q_per_img, len(EMOTION_QUESTIONS)))
        ...
        for question, choices in sampled_qs:
            ...
            try:
                if is_f2:
                    ...
                    messages = [{"role": "user", "content": [
                        {"type": "image"},
                        {"type": "text", "text": f"Look at the people in this image and answer: {question}"}
                    ]}]
            ...
            predicted = extract_emotion_answer(answer, choices)
    ...
    if verbose:
        print(f"\n{'=' * 60}")
        print(f"Emotion Detection Results: {display}")
        print(f"{'=' * 60}")
        print(f"  Questions answered:  {total}")
        print(f"  Skipped:             {skipped}")
        print(f"  FPS:                 {fps:.2f}")
        print(f"  Accuracy:            {accuracy:.4f} ({correct}/{total})")
        print(f"  Note: Template-based evaluation on COCO (no ground-truth emotion labels)")
    ...
    save_stats(stats, f"{safe_name}_emotion")
    return stats


def main():
    parser = argparse.ArgumentParser(description="Emotion Detection Benchmark (COCO)")
    all_choices = [m for m in list(MODEL_LOADERS) + list(MODEL_ALIASES)
                   if MODEL_ALIASES.get(m, m) in EMOTION_MODELS or m in EMOTION_MODELS]
    ...
    stats = benchmark_emotion(args.model, max_questions=args.max_questions, verbose=True)
    if stats:
        print_comparison({args.model: stats}, "EMOTION DETECTION — COCO")
